...
export default function DetailMoviePage() {
    ...
    // useEffect(() => {
    //     let logo = data?.heThongRapChieu.map((item, index) => {
    //         return item
    //     })
    //     setState({
    //         state: logo
                            ...
                            {item.cumRapChieu.map((item1, index) => {
                                return (
                                    <div key={index} className='theater-title'>
                                        <div>
                                            <p className='theater-name'>{item1.tenCumRap}</p>
                                        ...
                                        <div key={index} className=' mt-2 theater-movie-date' >
                                            {item1.lichChieuPhim.map((item2, index) => {
                                                return (
                                                    <Fragment key={index}>
                                                        <Link className='movie-date' >{item2.ngayChieuGioChieu}</Link>
                                                        <Link className='movie-date' >{item2.ngayChieuGioChieu}</Link>
                                                    </Fragment>
                                                )
                                            })}
    ...
    return (
        <div className='background-detail-movie'>
